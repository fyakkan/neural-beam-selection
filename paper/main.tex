...
this paper are publicly available.\footnote{\url{https://github.com/REPLACE-WITH-REPO-URL}}
...
pipeline. We emphasize that this is a faithful \emph{re-implementation} of the
regression benchmark rather than a re-run of shipped code: the reference
MathWorks example uses a wider $64$--$128$--$256$--$128$ ReLU network with
global-max input normalization on a $20{,}000/700$ split, whereas we use a
compact width-$96$ leaky-ReLU $z$-score MLP on the bundled $15{,}000/500$ dataset.
Despite these differences, our re-implementation reproduces the published
operating point ($90\%$ top-$K$ accuracy at $K=13$), confirming functional
equivalence.
...
i.i.d.\ model for simplicity and leave correlated-blockage robustness to future
...
and Signal Processing Toolboxes) on CPU.
